...
mha_fwd(at::Tensor q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
        ...
        std::optional<at::Tensor> q_ranges_,  // (b, 2)
        std::optional<at::Tensor> k_ranges_,  // (b, 2)
        std::optional<at::Tensor> cu_seqlens_q_,  // b+1
        std::optional<at::Tensor> cu_seqlens_k_,  // b+1
        std::optional<at::Tensor> cu_seqlens_k_new_,  // b+1
        std::optional<at::Tensor> seqused_q_, // b. If given, only this many elements of each batch element's queries and outputs are used.
        ...
        std::optional<at::Tensor> attn_type_map_, // b
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> mha_bwd(
    at::Tensor dout,  // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    at::Tensor q,     // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    at::Tensor k,     // (b, s_k, h_k, d) or (total_k, h_k, d) if there is cu_seqlens_k
    at::Tensor v,     // (b, s_k, h_k, d) or (total_k, h_k, d) if there is cu_seqlens_k
    at::Tensor out,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    ...
    std::optional<int64_t> max_seqlen_q_,
    std::optional<int64_t> max_seqlen_k_,
    std::optional<at::Tensor> attn_type_map_, // b. If given, only this many elements of each batch element's queries and outputs are used.
    std::optional<double> softmax_scale_,
    bool is_causal,
    int64_t window_size_left,
    int64_t window_size_right,
...
TORCH_LIBRARY(flexible_flash_attention, m) {
    m.def("fwd("
        "Tensor q,"
        "Tensor k,"
        "Tensor v,"
        ...
        "Tensor? q_v = None,"
        "Tensor(out!)? out = None,"
        "Tensor? q_ranges = None,"
        "Tensor? k_ranges = None,"
        ...
        "bool? pack_gqa = None,"
        "int sm_margin = 0,"
        "bool disable_fwd_atomic_reduction = False) -> (Tensor, Tensor, Tensor)");
    m.def("bwd("
        "Tensor dout,"
        "Tensor q,"
        "Tensor k,"
...
TORCH_LIBRARY_IMPL(flexible_flash_attention, CUDA, m) {
    m.impl("fwd", &mha_fwd);
    m.impl("bwd", &mha_bwd);

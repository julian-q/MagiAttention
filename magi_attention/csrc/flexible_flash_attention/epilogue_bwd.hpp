...
struct CollectiveEpilogueBwd {
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_dK, args.shape_dK, args.stride_dK, args.ptr_dV, args.stride_dV,
                tma_store_dK, tma_store_dV, args.cu_seqlens, args.ranges, args.seqused,
    ...
    store(Params const& params,
        ...
        flash::SeqlenInfoBwd<Varlen, kBlockN> seqlen_info{bidb, size<0>(params.shape_dK), params.cu_seqlens, params.ranges, params.seqused};
        bool const is_varlen = Varlen && (params.cu_seqlens || params.ranges);

        Tensor mdK = params.tma_store_dK.get_tma_tensor(params.shape_dK)(_, _, bidh_kv, !is_varlen ? bidb : 0);
        Tensor mdV = params.tma_store_dV.get_tma_tensor(params.shape_dK)(_, _, bidh_kv, !is_varlen ? bidb : 0);
        Tensor gdK = local_tile(domain_offset(make_coord(seqlen_info.offset, _0{}), mdK), select<1, 2>(TileShape_MNK{}), make_coord(n_block, _0{}));  // (M, K)
        Tensor gdV = local_tile(domain_offset(make_coord(seqlen_info.offset, _0{}), mdV), select<1, 2>(TileShape_MNK{}), make_coord(n_block, _0{}));  // (M, K)
    ...
    store_zero(
        ...
        // flash::SeqlenInfoBwd<Varlen, kBlockN> seqlen_info{bidb, size<0>(params.shape_dK), params.cu_seqlens, params.ranges, params.seqused};
        // bool const is_varlen = Varlen && (params.cu_seqlens || params.ranges);
        // Tensor mdK = make_tensor(make_gmem_ptr(params.ptr_dK), params.shape_dK, params.stride_dK)(_, _, bidh_kv, !is_varlen ? bidb : 0);
        ...
        // Tensor mdV = make_tensor(make_gmem_ptr(params.ptr_dV), params.shape_dK, params.stride_dV)(_, _, bidh_kv, !is_varlen ? bidb : 0);
...
struct CollectiveEpilogueBwdGQA {
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_dKaccum, args.shape_dKaccum, args.stride_dKaccum, args.ptr_dVaccum, args.stride_dVaccum,
                ...
                args.cu_seqlens, args.ranges, args.seqused};
    }
    ...
    store(Params const& params,
        ...
        flash::SeqlenInfoBwd<Varlen, kBlockN> seqlen_info{bidb, size<0>(params.shape_dKaccum), params.cu_seqlens, params.ranges, params.seqused};
        bool const is_varlen = Varlen && (params.cu_seqlens || params.ranges);
        ...
        Tensor gdKaccum = local_tile(domain_offset(make_coord(seqlen_info.offset * kHeadDim), mdKaccum), Shape<Int<kBlockN * kHeadDim>>{}, make_coord(n_block));  // (M * K)
        Tensor gdVaccum = local_tile(domain_offset(make_coord(seqlen_info.offset * kHeadDim), mdVaccum), Shape<Int<kBlockN * kHeadDim>>{}, make_coord(n_block));  // (M * K)
        ...
        if constexpr (Use_TMA) {
            ...
            if (thread_idx == 0) {
                SM90_BULK_REDUCE_ADD::copy(raw_pointer_cast(sdKV_flat.data()), raw_pointer_cast(gdVaccum.data()), dKV_TMA_num_bytes);
                tma_store_arrive();
                tma_store_wait<0>();
            }
        } else {

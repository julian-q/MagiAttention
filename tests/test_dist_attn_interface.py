...
import dffa
import dffa.testing
from dffa.api.dist_flex_attn_interface import (
    DistAttnRuntimeDict,
    calc_attn,
...
from dffa.api.functools import (
    compute_pad_size,
...
class TestInterfacePipelineSDPABaseWithWorldSize1(DistTestBase):
    ...
    @parameterize(
        ...
        [
            ...
            # varlen full attn with total seqlen 1050
            {
                ...
                SKIP_WORLD_SIZE: [4, 8],
                INTERFACE: "dist_flash_attn_flex",
                "q_ranges": AttnRanges.from_ranges(
                    [
                        [0, 128],
                        [128, 256],
            ...
            },
            {
                NAME: "varlen_full_attn_1050",
                SKIP_WORLD_SIZE: [4, 8],
                INTERFACE: "dist_flash_attn_varlen",
    ...
    def test_pipeline_sdpa(
        ...
        if interface == "dist_flash_attn":
            ...
            cu_seqlens_k, cu_seqlens_q = full_attention_to_varlen_attention(
                batch_size, attn_config["total_seqlen_q"] // batch_size
            )

            x_padded, dist_attn_runtime_key = dist_flash_attn_varlen_dispatch(
                x,
                cu_seqlens_q,
                cu_seqlens_k,
                head_dim=head_dim,
        ...
        if interface == "dist_flash_attn_varlen":
            ...
            x_padded, dist_attn_runtime_key = dist_flash_attn_flex_dispatch(
                x,
                q_ranges=q_ranges,
                k_ranges=k_ranges,

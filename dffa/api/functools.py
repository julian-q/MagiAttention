...
def compute_pad_size(total_seqlen_q, cp_size, head_dim):
    ...
    dim_block_map = {64: 128, 128: 80, 256: 64}
    for dim in sorted(dim_block_map):
        if head_dim <= dim:
            head_dim = dim
            q_block_size = dim_block_map[dim]
            break

    # Validate sequence length
    block_requirement = q_block_size * cp_size
    tokens_to_pad = 0
    if (remainder := total_seqlen_q % block_requirement) != 0:
        tokens_to_pad = block_requirement - remainder

    return tokens_to_pad, q_block_size

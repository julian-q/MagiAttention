...
class ImagenetDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "fit" or stage is None:
            ...
            self.dataset_train = ImageFolder(
                self.data_dir / "train", transform=train_transforms
            )
            self.dataset_val = ImageFolder(
                self.data_dir / "val", transform=val_transforms
            )
        ...
        if stage == "test" or stage is None:
            ...
            self.dataset_test = ImageFolder(
                self.data_dir / "val", transform=test_transforms
            )
    ...
    def val_dataloader(
        ...
        if not self.cache_val_dataset:
            sampler = (
                DistributedSampler(
                    self.dataset_val, shuffle=False, drop_last=self.drop_last
                )
                if self.num_aug_repeats != 0
                else None
            )
            return self._data_loader(
                self.dataset_val, batch_size=self.batch_size_eval, sampler=sampler
            )
        else:
            ...
            self.dataset_val = DictDataset(
                dict(zip(indices, batches)), length=len(self.dataset_val)
            )
            sampler = (
                DistributedSampler(
                    self.dataset_val, shuffle=False, drop_last=self.drop_last
    ...
    def test_dataloader(
        ...
        sampler = (
            DistributedSampler(
                self.dataset_test, shuffle=False, drop_last=self.drop_last
            )
            if self.num_aug_repeats != 0
            else None
        )
        return self._data_loader(
            self.dataset_test, batch_size=self.batch_size_eval, sampler=sampler
        )
    ...
    def _data_loader(
        ...
        collate_fn = (
            (lambda batch: mixup(*default_collate(batch)))

...
def pooling(x, pooling_mode="CLS", key_padding_mask=None, batch_first=True):
    ...
    if pooling_mode in ["MEAN", "SUM"]:
        ...
        return rearrange(
            x, "b ... -> b (...)" if batch_first else "s b ... -> b (s ...)"
        )
...
class ClassificationHeadLinear(nn.Module):
    ...
    def __init__(
        self, d_model, num_classes, pooling_mode="MEAN", batch_first=False, **kwargs
    ):
        super().__init__()
...
class ClassificationHeadDual(nn.Module):
    ...
    def __init__(
        ...
        self.dense = nn.Linear(
            d_model * (4 if self.interaction == "NLI" else 2), d_inner
        )
        self.dropout = nn.Dropout(dropout)
        self.out_proj = nn.Linear(d_inner, num_classes)
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = (
            torch.cat([x1, x2, x1 * x2, x1 - x2], dim=-1)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(
        self, d_model, dropout=0.1, max_len=5000, batch_first=False, initializer=None
    ):
        super().__init__()
        self.batch_first = batch_first

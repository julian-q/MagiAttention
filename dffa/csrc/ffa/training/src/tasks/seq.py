...
class SequenceModel(LightningModule):
    ...
    def step(self, batch: Any, is_train=True):
        try:
            ...
            lengths = None
        output = (
            self.forward(x) if lengths is None else self.forward(x, lengths=lengths)
        )
    ...
    def shared_step(self, batch: Any, batch_idx: int, phase="train"):
        ...
        log_on_step = (
            "eval" in self.cfg
            and self.cfg.eval.get("log_on_step", False)
            and phase == "train"
        )
        self.log(
            f"{phase}/loss",
        ...
        self.log_dict(
            metrics, on_step=log_on_step, on_epoch=True, prog_bar=True, sync_dist=True
        )

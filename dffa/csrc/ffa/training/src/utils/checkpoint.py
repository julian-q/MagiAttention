...
def blockdiag_to_dense_mlp_bert(state_dict):
    ...
    names = {
        ...
        if re.match(
            "bert.encoder.layer.(\d+).(mlp.fc(1|2)|(intermediate|output).dense).weight",
            name,
        )
    }
    for name in names:
...
def interpolate_pos_embedding(
    ...
    if interleave:
        ...
        assert (
            math.isqrt(out_seqlen) ** 2 == out_seqlen
        ), "interleave only works for square lengths"
        assert (
            math.isqrt(reps[1]) ** 2 == reps[1]
        ), "out_seqlen / seqlen must be a perfect square"

        emb_square = rearrange(
            orig_emb, "b (h w) d -> b h w d", h=math.isqrt(orig_emb.shape[1])
        )

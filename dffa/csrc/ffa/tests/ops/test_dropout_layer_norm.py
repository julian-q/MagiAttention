...
def test_dropout_layer_norm_training(
    ...
    our_layer_norm_func = (
        dropout_add_layer_norm if not is_rms_norm else dropout_add_rms_norm
    )
    device = "cuda"
    ...
    model = our_layer_norm_cls(
        hidden_size, p=dropout_p, device=device, dtype=weight_dtype
    )
    with torch.no_grad():
    ...
    out.backward(g)
    out_ref.backward(g)
    assert (x0.grad - x0_ref.grad).abs().max() <= 4 * (
        x0_pt.grad - x0_ref.grad
    ).abs().max() + 1e-4
    if has_residual:
...
@pytest.mark.parametrize(
    "hidden_size", [768, 1024, 1280, 1536, 1600, 2048, 2560, 3072, 4096, 5120]
)
...
def test_dropout_layer_norm_prenorm_eval(
    hidden_size, input_dtype, residual_dtype, weight_dtype
):
    if weight_dtype == torch.float16 and input_dtype == torch.bfloat16:
        pytest.skip()  # Not supported
...
def test_dropout_layer_norm_subset_training(
    ...
    if has_residual:
        residual_pt = (
            (x0_scaled_pt.float() * dmask_expanded.float()) / (1 - dropout_p)
            + res_pt.float()
        ).to(dtype=residual_dtype)
        residual_ref = (x0_scaled_ref * dmask_expanded.float()) / (
            1 - dropout_p
        ) + res_ref
    else:
        residual_pt = (
    ...
    out.backward(g)
    out_ref.backward(g)
    assert (x0.grad - x0_ref.grad[x0_mask_batch]).abs().max() <= 4 * (
        x0_pt.grad - x0_ref.grad
    )[x0_mask_batch].abs().max() + 1e-4
    if has_residual:
        assert (res.grad - res_ref.grad).abs().max() <= 4 * (
            res_pt.grad - res_ref.grad
        ).abs().max() + 1e-4
...
def test_dropout_layer_norm_subset_prenorm_training(
    ...
    g = torch.randn_like(out) / batch_size
    (
        out_pt * F.sigmoid(residual_pt[out_mask_batch])
        + residual_pt.mean(0, keepdim=True)
    ).backward(g)
    (
        out * F.sigmoid(residual[out_mask_batch]) + residual.mean(0, keepdim=True)
    ).backward(g)
    (
...
def test_dropout_layer_norm_parallel_residual_training(
    ...
    bias0_ref = (
        bias0.detach().clone().float().requires_grad_() if bias0 is not None else None
    )
    if not tied_norm:
        weight1 = torch.randn(
            hidden_size, device=device, dtype=weight_dtype, requires_grad=True
        ...
        bias1_ref = (
            bias1.detach().clone().float().requires_grad_()
    ...
    if has_residual:
        if has_x1:
            ...
            residual_pt = (
                (x0_pt.float() * dmask0.float()) / (1 - dropout_p) + res_pt.float()
            ).to(dtype=residual_dtype)
    ...
    if not is_rms_norm:
        ...
        out0_ref = F.layer_norm(
            residual_ref, (hidden_size,), weight0_ref, bias0_ref, eps=epsilon
        )
        if not tied_norm:
        ...
        out0_ref = fused_rms_norm_affine(
            residual_ref, weight0_ref, (hidden_size,), eps=epsilon
        )
        if not tied_norm:
            ...
            out1_ref = fused_rms_norm_affine(
                residual_ref, weight1_ref, (hidden_size,), eps=epsilon
            )

    assert (out0 - out0_ref).abs().max() <= 4 * (out0_pt - out0_ref).abs().max() + 1e-4
    if not tied_norm:
        assert (out1 - out1_ref).abs().max() <= 4 * (
            out1_pt - out1_ref
        ).abs().max() + 1e-4

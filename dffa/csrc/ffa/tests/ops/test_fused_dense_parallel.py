...
def test_fused_linear_bias(
    ...
    out_pt.backward(g)
    out.backward(
        g[:, rank * partition_out_features : (rank + 1) * partition_out_features]
    )
...
def test_fused_mlp(
    in_features, out_features, has_bias2, sequence_parallel, world_size, dtype
):
    assert out_features % world_size == 0
    rtol, atol = (3e-3, 3e-2) if dtype == torch.bfloat16 else (3e-3, 3e-3)
    if not torch.distributed.is_initialized():

...
def attention_forward_core_ref_impl(q, k, v, sm_scale, causal, use_exp2):
    ...
    if causal:
        ...
        if DEBUG:
            print(
                "attention_scaled_scores after causal:",
                attention_scaled_scores,
                attention_scaled_scores.shape,
            )
...
def attention_vanilla_forward_pytorch_ref_impl(
    q, k, v, sm_scale, causal, layout, use_exp2
):
    ...
    (
        ...
        attention_scaled_scores,
        attention_scores,
    ) = attention_forward_core_ref_impl(q, k, v, sm_scale, causal, use_exp2)
...
def attention_varlen_forward_pytorch_ref_impl(
    ...
    softmax_lse = torch.empty(
        (total_L_q, num_heads), dtype=torch.float32, device=q.device
    )
    ...
    for i in range(batch_size):
        ...
        o[start_q:end_q, :, :] = o_i
        softmax_lse[start_q:end_q, :] = softmax_lse_i.transpose(
            0, 1
        )  # Transpose to [L_q_i, num_heads]
        ...
        attention_shifted_scaled_scores_i = attention_shifted_scaled_scores_i.permute(
            1, 0, 2
        )
...
def attention_forward_pytorch_ref_impl(
    ...
    if DEBUG:
        ...
        print(
            "exp_scores_ref:",
            exp_scores_ref,
            exp_scores_ref.shape if exp_scores_ref is not None else None,
        )

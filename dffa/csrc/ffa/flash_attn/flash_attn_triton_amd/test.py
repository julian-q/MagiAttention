...
from .interface_torch import attention_decode, attention_prefill
from .utils import DEBUG, MetaData, get_input_shapes, input_helper, varlen_input_helper
...
def test_op_fwd_prefill(
    ...
    if causal:
        mask = torch.tril(
            torch.ones(N_CTX_Q, N_CTX_K, device="cuda"), diagonal=N_CTX_K - N_CTX_Q
        )
...
def test_op_fwd_prefill_bias(
    Z, H, N_CTX_Q, N_CTX_K, D_HEAD, causal, use_bias, dtype=torch.float16
):
    torch.manual_seed(20)
    sm_scale = D_HEAD**-0.5
    input_metadata = MetaData(sm_scale=sm_scale)
    q, k, v, input_metadata = input_helper(
        Z, H, H, N_CTX_Q, N_CTX_K, D_HEAD, dtype, layout="bhsd"
    )
    if causal:
        input_metadata.need_causal()
    if use_bias:
...
def test_op_varlen_fwd(Z, H, N_CTX, D_HEAD, causal, dtype=torch.float16):
    ...
    for i in range(0, input_metadata.num_contexts):
        start_q, start_k = (
            input_metadata.cu_seqlens_q[i],
            input_metadata.cu_seqlens_k[i],
        )
        end_q, end_k = (
            input_metadata.cu_seqlens_q[i + 1],
...
def test_op_bwd(
    ...
    if DEBUG_INPUT:
        q = (
            torch.arange(seqlen_q, dtype=dtype, device="cuda")
            .view(1, 1, seqlen_q, 1)
            .expand(Z, H, seqlen_q, D_HEAD)
            .requires_grad_()
        )
        k = (
            torch.arange(seqlen_k, dtype=dtype, device="cuda")
            .view(1, 1, seqlen_k, 1)
            .expand(Z, H, seqlen_k, D_HEAD)
            .requires_grad_()
        )
        v = (
            torch.arange(seqlen_k, dtype=dtype, device="cuda")
        ...
        q = torch.randn(
            Z, H, N_CTX_Q, D_HEAD, device="cuda", dtype=dtype, requires_grad=True
        )
        k = torch.randn(
            Z, H, N_CTX_K, D_HEAD, device="cuda", dtype=dtype, requires_grad=True
        )
        v = torch.randn(
            Z, H, N_CTX_K, D_HEAD, device="cuda", dtype=dtype, requires_grad=True
        )
...
@pytest.mark.parametrize(
    "DEBUG_INPUT", [False]
)  # NOTE: debug input can overflow when the tensors are large. Just use to figure out issues
def test_op_prefill_fwd_impl(
    Z, H, N_CTX_Q, N_CTX_K, D_HEAD, causal, return_scores, layout, use_exp2, DEBUG_INPUT
):
    dtype = torch.float16
    torch.manual_seed(0)
    alibi_slopes = None
    ...
    torch.testing.assert_close(
        softmax_lse_triton, softmax_lse_ref, atol=ATOL, rtol=RTOL
    )

    if layout != "thd":
        # use trick with lse to get the softmax. you need the scores but is it
        softmax_triton = torch.exp(
            attention_scaled_scores_ref - softmax_lse_triton.unsqueeze(-1)
        )
        if DEBUG:
            print(
                "attention_scaled_scores_ref:",
                attention_scaled_scores_ref,
                attention_scaled_scores_ref.shape,
            )
            print("softmax_lse_triton:", softmax_lse_triton, softmax_lse_triton.shape)
    ...
    if False and layout in ["bhsd", "bshd"] and not causal:
        ...
        torch.testing.assert_close(
            softmax_triton, softmax_pytorch.to(torch.float32), atol=ATOL, rtol=RTOL
        )
...
@pytest.mark.parametrize(
    "use_exp2", [False]
)  # FIXME: using exp2 causes issue when used with causal
@pytest.mark.parametrize("layout", ["bhsd", "bshd", "thd"])
@pytest.mark.parametrize("sequence_parallel", [True, False])
@pytest.mark.parametrize(
    "DEBUG_INPUT", [False]
)  # debug output causes nans in both new and old backend
def test_op_prefill_bwd_impl(
    ...
    if layout == "thd":
        q, k, v, metadata = varlen_input_helper(
            Z, H, H, N_CTX_Q, N_CTX_K, D_HEAD, dtype, DEBUG_INPUT=DEBUG_INPUT
        )
    else:
        q, k, v, metadata = input_helper(
            Z, H, H, N_CTX_Q, N_CTX_K, D_HEAD, dtype, layout, DEBUG_INPUT=DEBUG_INPUT
        )
    if DEBUG_INPUT:
    ...
    dq = torch.zeros_like(
        q, dtype=q.dtype
    )  # NOTE: the kernel does inplace accumlation on dq so dq has to be zeros
    if DEBUG_INPUT:
    ...
    torch.testing.assert_close(
        delta_triton, delta_ref, atol=ATOL, rtol=RTOL, equal_nan=EQUAL_NAN
    )
    ...
    torch.testing.assert_close(
        dv_triton, dv_ref, atol=ATOL, rtol=RTOL, equal_nan=EQUAL_NAN
    )
    ...
    torch.testing.assert_close(
        dk_triton, dk_ref, atol=ATOL, rtol=RTOL, equal_nan=EQUAL_NAN
    )
    ...
    torch.testing.assert_close(
        dq_triton, dq_ref, atol=ATOL, rtol=RTOL, equal_nan=EQUAL_NAN
    )


@pytest.mark.parametrize(
    "batch_size, seqlen_q, seqlen_k, group_q, group_k, dim", get_input_shapes()
)
def test_op_fwd_decode(
    batch_size, seqlen_q, seqlen_k, group_q, group_k, dim, dtype=torch.bfloat16
):
    if DEBUG:
        print()
        print(
            f"batch_size = {batch_size}, seqlen_q = {seqlen_q}, seqlen_k = {seqlen_k}, group_q = {group_q}, group_k = {group_k}, dim = {dim}"
        )
    torch.manual_seed(20)
    query_group_head_size = (group_q + group_k - 1) // group_k
    q = (
        torch.empty(
            (batch_size, seqlen_q, group_k, query_group_head_size, dim),
            dtype=dtype,
            device="cuda",
        )
    ...
    k = (
        torch.empty((batch_size, seqlen_k, group_k, 1, dim), dtype=dtype, device="cuda")
        .normal_(mean=0.0, std=0.5)
        .requires_grad_()
    ).expand(-1, -1, -1, query_group_head_size, -1)
    v = (
        torch.empty((batch_size, seqlen_k, group_k, 1, dim), dtype=dtype, device="cuda")
        .normal_(mean=0.0, std=0.5)
        .requires_grad_()

...
import torch
import triton
from flash_attn.flash_attn_triton_amd.interface_torch import (
    attention_decode,
    attention_prefill,
)
...
FUNCTIONS = {"prefill": attention_prefill, "decode": attention_decode}
...
def gen_fn_inputs(
    fn_name, BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, device, layout, causal
):
    flops_per_matmul = 0
    ...
    if fn_name.startswith("prefill"):
        if layout == "thd":
            ...
            for i in range(input_metadata.num_contexts):
                seqlen_q = (
                    input_metadata.cu_seqlens_q[i + 1] - input_metadata.cu_seqlens_q[i]
                )
                seqlen_k = (
                    input_metadata.cu_seqlens_k[i + 1] - input_metadata.cu_seqlens_k[i]
                )
...
def main():
    ...
    assert (
        args.dtype in ARGS_TO_TORCH_DTYPE
    ...
    for fn_name in bench_fn_list:
        ...
        for mode in args.mode:
            if fn_name == "decode" and mode == "bwd":
                print("Decode kernel doesnot have a backward pass")
                continue

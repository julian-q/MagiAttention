...
from .utils import DEBUG, get_shape_from_layout, get_strides_from_layout
...
def _bwd_preprocess_use_o(
    ...
    # load
    o = tl.load(out_ptrs, mask=mask_m[:, None] & mask_d[None, :], other=0.0).to(
        tl.float32
    )
    do = tl.load(do_ptrs, mask=mask_m[:, None] & mask_d[None, :], other=0.0).to(
        tl.float32
    )
...
def _bwd_kernel(
    ...
    l_offset = (
        L + off_z * stride_deltaz + off_h * stride_deltah + q_start * stride_deltam
    )
    d_offset = (
        D + off_z * stride_deltaz + off_h * stride_deltah + q_start * stride_deltam
    )

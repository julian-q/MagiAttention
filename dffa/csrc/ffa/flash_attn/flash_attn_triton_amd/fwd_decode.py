...
def _fwd_kernel_splitK(
    ...
    k_base = (
        K + k_head_idx * stride_kh + cache_batch_idx * stride_kz + off_g_q * stride_kg
    )
    v_base = (
        V + v_head_idx * stride_vh + cache_batch_idx * stride_vz + off_g_q * stride_vg
    )
    ...
    q = tl.load(tl.advance(Q_block_ptr, (0, 0)), boundary_check=(0,))  # noqa: F821
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        if IS_CAUSAL:
            alpha = tl.math.exp2(
                tl.where(m_i > float("-inf"), m_i - m_i_new, float("-inf"))
            )
        else:
...
def _splitK_reduce(
    ...
    Metadata_ptr = (
        Metadata + stride_mzhg * off_zhg + spk_idx * stride_ms + off_m * stride_mm
    )
...
def attention_decode_forward_triton_impl(
    ...
    assert (
        dim_k == dim_q
    ), f"Keys have head dim {dim_k} but queries have head dim {dim_q}"
    ...
    if SPLIT_K is not None:
        ...
        # Use heuristics
        split_k = get_split_k(
            batch_size, n_group_q, heads_per_group_q, seqlen_k
    ...
    out = torch.empty(
        (batch_size, seqlen_q, n_group_q, heads_per_group_q, dim_padded),
        device=q.device,
        dtype=q.dtype,
    )
    ...
    if seqlen_k == 0:
        out.zero_()
    out = out.reshape(
        batch_size, heads_per_group_q * n_group_q, -1, dim_padded
    ).contiguous()

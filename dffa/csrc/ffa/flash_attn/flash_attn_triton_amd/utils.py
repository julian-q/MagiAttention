...
def input_helper(
    Z, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, layout, device="cuda", DEBUG_INPUT=False
):
    ...
    if DEBUG_INPUT:
        if layout == "bhsd":
            q = (
                torch.arange(N_CTX_Q, dtype=dtype, device=device)
                .view(1, 1, N_CTX_Q, 1)
                .expand(*q_tensor_shape)
                .contiguous()
                .requires_grad_()
            )
            k = (
                torch.arange(N_CTX_K, dtype=dtype, device=device)
                .view(1, 1, N_CTX_K, 1)
                .expand(*k_tensor_shape)
                .contiguous()
                .requires_grad_()
            )
            v = (
...
def varlen_input_helper(
    ...
    cu_seqlens_q = torch.cat(
        [torch.tensor([0], dtype=torch.int32), seqlens_q.cumsum(dim=0)]
    )
    cu_seqlens_k = torch.cat(
        [torch.tensor([0], dtype=torch.int32), seqlens_k.cumsum(dim=0)]
    )
    ...
    if DEBUG_INPUT:
        ...
        q = torch.randn(
            (total_q, HQ, D_HEAD), dtype=dtype, device=device
        ).requires_grad_()
        k = torch.randn(
            (total_k, HK, D_HEAD), dtype=dtype, device=device
        ).requires_grad_()
        v = torch.randn(
            (total_k, HK, D_HEAD), dtype=dtype, device=device
        ).requires_grad_()

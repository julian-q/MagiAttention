...
def remap_state_dict_hf_btlm(state_dict, config):
    ...
    if "transformer.wpe.weight" in state_dict:
        state_dict = OrderedDict(
            (key_mapping_pos_emb(k), v) for k, v in state_dict.items()
        )
    ...
    for d in range(config.num_hidden_layers):
        ...
        state_dict[f"transformer.layers.{d}.mlp.fc1.weight"] = torch.cat(
            [W1.t(), W3.t()], dim=0
        )
    ...
    def key_mapping_attn(key):
        key = re.sub(
            ...
            r"transformer.layers.\1.mixer.Wqkv.bias",
            key,
        )
        key = re.sub(
...
def btlm_config_to_gpt2_config(btlm_config: PretrainedConfig) -> GPT2Config:
    return GPT2Config(
        ...
        n_positions=0
        if btlm_config.position_embedding_type == "alibi"
        else btlm_config.n_positions,
        ...
        use_flash_attn=btlm_config.position_embedding_type
        == "alibi",  # Alibi code path requires flash_attn

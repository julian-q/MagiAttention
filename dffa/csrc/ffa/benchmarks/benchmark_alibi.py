...
def attention_pytorch(q, k, v, dropout_p=0.0, causal=True, attn_bias=None):
    ...
    if attn_bias is not None:
        ...
        scores = torch.empty(
            batch_size * nheads, seqlen, seqlen, dtype=q.dtype, device=q.device
        )
    scores = rearrange(
        torch.baddbmm(scores, q, k, beta=1.0, alpha=softmax_scale),
        "(b h) t s -> b h t s",
        h=nheads,
    )
    if causal:
...
for causal in causal_vals:
    for headdim in headdim_vals:
        for batch_size, seqlen in bs_seqlen_vals:
            ...
            alibi_slopes = (
                torch.rand(1, nheads, device=device, dtype=torch.float32) * 0.3
            )
            ...
            if xops is not None:
                ...
                v = v.detach().requires_grad_(True)
                if causal:
                    attn_bias_xops = xops.LowerTriangularMask().add_bias(
                        attn_bias.expand(-1, -1, seqlen, -1).to(dtype=q.dtype)
                    )
                    ...
                    #     attn_bias type is <class 'xformers.ops.fmha.attn_bias.LowerTriangularMaskWithTensorBias'>
                    attn_bias_xops = attn_bias_xops.materialize(
            ...
            f, b = time_fwd_bwd(
                flash_rotary, q, k, v, cos, sin, causal, repeats=repeats, verbose=False
            )

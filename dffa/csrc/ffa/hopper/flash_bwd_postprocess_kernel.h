...
public:
    ...
    operator()(Params const& params, char* smem_buf) {
        ...
        flash::SeqlenInfoBwd<true /*Varlen*/, kBlockM> seqlen_info(bidb, size<0>(params.shape_dQ), params.cu_seqlens, params.ranges, params.seqused);
        bool const is_varlen = (params.cu_seqlens || params.ranges);
        ...
        Tensor gdQaccum = local_tile(domain_offset(make_coord(seqlen_info.offset * kHeadDim), mdQaccum), Shape<Int<kBlockM * kHeadDim>>{}, make_coord(m_block));  // (M * K)
        if (thread_idx == 0 && m_block == 0 && bidh == 0) {
            printf("seqlen_info.offset: %d\n", seqlen_info.offset);
        }

...
struct CollectiveEpilogueFwd {
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_O, args.shape_O, args.stride_O, shape_O_packed, stride_O_packed,
                ...
                tma_store_O, args.nheads, args.range_locks, args.cu_seqlens, args.q_ranges, args.seqused};
    }
    ...
    void acquire_lock(int* range_lock, int bidh, int offset, int q_block_size, int num_heads) {
        ...
        // printf("loop_count: %d, bidh: %d, offset: %d, q_block_size: %d, num_heads: %d\n", loop_count, bidh, offset, q_block_size, num_heads);
    ...
    store(Params const& params,
        ...
        auto gr_tiled_copy_O = make_tiled_copy_C(Copy_Atom<DefaultCopy, ElementPartial>{}, tiled_mma);
        ...
        // load prev_output
        if constexpr (!PackGQA && !Use_TMA_O && !is_split) {
        }
        else {
            // clean code
        }
        ...
        // Tensor tOrCurrO = make_fragment_like(tOsO);
        // cute::copy(gmem_tiled_copy_O, tOsO, tOrCurrO);
        // if constexpr (ArchTag::kMinComputeCapability >= 90) {
        ...
        //     gmem_tiled_copy_O, tOrCurrO, tOgO, tOcO, tOpO, seqlen_o - m_block * kBlockM
        // );
